...
polys_metal = top.get_polygons(
    apply_repetitions=True,
    include_paths=True,
    depth=None,
    layer=LAYER_METAL,
    datatype=DATATYPE_METAL,
)

print(f"Found {len(polys)} polygons on layer/datatype = ({LAYER}, {DATATYPE})")
print(f"Found {len(polys_metal)} polygons on layer/datatype = ({LAYER_METAL}, {DATATYPE_METAL})")
...
meshes_metal = []
for i, poly in enumerate(polys_metal):
    xy = np.asarray(poly.points if hasattr(poly, "points") else poly)

    try:
        mesh_metal = polygon_to_extruded_mesh(xy, z0_metal, z1_metal)
        meshes_metal.append(mesh_metal)
    except Exception as e:
        print(f"skip polygon {i}: {e}")
...
print(f"Metal layer Valid meshes: {len(meshes_metal)}")
...
for mesh_metal in meshes_metal:
    pl.add_mesh(
        mesh_metal,
        color=colors["metal"],
        opacity=opacity["metal"],
        show_edges=False,
        # smooth_shading=True,
        # ambient=0.85,
        # diffuse=0.2,
